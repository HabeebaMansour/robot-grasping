...
class PyBulletSim:
    ...
    def __init__(self, use_random_objects=False, object_shapes=None, gui=True):
        ...
        self.move_joints(self.robot_home_joint_config, speed=0.01)
    ...
    def move_tool(self, position, orientation, speed=0.01):
        """
        ...
        target_joint_state = p.calculateInverseKinematics(self.robot_body_id, self.robot_end_effector_link_index, position, orientation, residualThreshold= 1e-6, maxNumIterations= 100)
        # ===============================
        self.move_joints(target_joint_state, speed)

    def get_grasp_position_angle(self, object_id):
        position, grasp_angle = np.zeros(3), 0
        q_grasp_angle = np.zeros(4)
        grasp_angles = np.zeros(3)
        # ========= TODO: Problem 2 ============
        # Get position and orientation (yaw in radians) of the gripper for grasping
        position, q_grasp_angle=p.getBasePositionAndOrientation(object_id)
        grasp_angles = p.getEulerFromQuaternion(q_grasp_angle)
        grasp_angle=grasp_angles[2]
        # ==================================
        return position, grasp_angle

    def robot_go_home(self, speed=0.01):
    ...
    def execute_grasp(self, grasp_position, grasp_angle):
        ...
        self.open_gripper()
        self.move_tool(pre_grasp_position_over_bin, gripper_orientation, speed)
        self.move_tool(pre_grasp_position_over_object, gripper_orientation, speed)
        self.move_tool(grasp_position, gripper_orientation, speed)
        self.close_gripper()
        self.move_tool(post_grasp_position, gripper_orientation, speed)
        self.robot_go_home()
    ...
    def check_collision(self, q): #used to be 0.18
